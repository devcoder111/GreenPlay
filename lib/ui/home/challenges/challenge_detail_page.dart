...
class ChallengeDetailScreen extends StatefulWidget {
  ...
  State<StatefulWidget> createState() {
    return _ChallengeDetailScreenState();
  }
}
  ...
  Widget _acceptChallengeButton() {
    return
      _buttonAccept == 1? Container(
        ...
        width: MediaQuery.of(context).size.width,
        child: RaisedButton(
          color: store.state.challengeListResponse.data[_index].isAccepted ==
              2
              ? Colors.red : AppColors.colorBlue,
          child: Text(
            store.state.challengeListResponse.data[_index].isAccepted ==
                2
                ? DemoLocalizations.of(context).trans('withdraw') :
            store.state.challengeListResponse.data[_index].isAccepted ==
                3
...
//                store.state.challengeListResponse.data.removeAt(_index);
              await _addChallengeParticipant().then((int response)async {
                if (response == 1) {
                  await _addChallengeUser().then((int response) async{
                    if (response == 1) {
                      ...
                      Future.delayed(Duration.zero, () => store.dispatch(MyChallengeApiAction()));
//                      await store.dispatch(MyChallengeApiAction());
                    }
                  });
                }
              });
            }else if(store.state.challengeListResponse.data[_index].isAccepted == 2){
              //withdraw function
              setState(() {
                isLoader = true;
              });
              await _withdrawChallengeUser("3").then((int response)async {
                //add to withdrawen case, again user has to accept challenge
                if (response == 1) {
                  setState(() {
                    store.state.challengeListResponse.data[_index].isAccepted =
                    3;
...
//                  await store.dispatch(MyChallengeApiAction());
            ...
            }else if(store.state.challengeListResponse.data[_index].isAccepted == 3){
              //withdraw function
              setState(() {
                isLoader = true;
              });
              await _withdrawChallengeUser("2").then((int response)async {
                //add again to accepted challenge list
                if (response == 1) {
                  setState(() {
                    store.state.challengeListResponse.data[_index].isAccepted =
                    2;
...
//                  await store.dispatch(MyChallengeApiAction());
            ...
            Container(
              ...
              Text(
                DemoLocalizations.of(context).trans('from') +" "+ GetMonth.getMonth
                  (store.state.challengeListResponse.data[_index].startDate.toString().toString(),context )
                    +" " + DemoLocalizations.of(context).trans('to')
                    + " "+GetMonth.getMonth
                  (store.state.challengeListResponse.data[_index].
                endDate.toString(),context )
                /*DemoLocalizations.of(context).trans('from') + new
                DateFormat("dd MMMM yyyy").format(DateTime.parse(
                store.state.challengeListResponse.data[_index].startDate.toString()))
                    +" "+ DemoLocalizations.of(context).trans('to') +
      ...
      _database
        ...
        try {
          ...
          challengeMap.forEach((user) {
            ...
            _modalChallenge.withdraw = user['withdraw'] ?? "2";
            challengeData
                .add(_modalChallenge);
          });
  ...
  Future<int> _withdrawChallengeUser(String withdrawnStatus) async {
    List<ChallengeData> challengeData = new List();
    ...
    try {
      _database
        ...
        try {
          ...
          challengeMap.forEach((user) async{
            if(user['challengeId'].toString() == store.state.challengeListResponse.data[_index].idChallenge){
              ChallengeData _modalChallenge = new ChallengeData();
              _modalChallenge.challengeName = user['challengeName'] ?? '';
              _modalChallenge.challengeId = user['challengeId'].toString();
              _modalChallenge.challengeStartDate = user['StartDate'] ?? '';
              ...
              _modalChallenge.challengeCreatedBy = user['createdBy'];
              _modalChallenge.scheduleType = user['ScheduleType'];
              _modalChallenge.withdraw = withdrawnStatus;
              challengeData
                  .add(_modalChallenge);
              MyChallengeAddModal _modalAddChallengeUser = new MyChallengeAddModal(store.state.userAppModal.userId,
                  store.state.userAppModal.firstName, store.state.userAppModal.lastName, challengeData);
    ...
    once().then((snapshot )async{
      if (snapshot .value != null){
        ...
        userData.forEach((user) {
          ...
          var challengeWithdraw = user['withdraw']?? "0";
          print("withdraw $challengeWithdraw");
          print(challengeWithdraw+" :...");

          //1 >> if accepted
          //2>>not yet withdrawen
          //3>>withdrawen
          //0>>nothing done....challenge not registered with user
          if(challengeId.toString() == store.state.challengeListResponse.data[_index].idChallenge.toString()){
            setState(() {
              store.state.challengeListResponse.data[_index].isAccepted =
                  int.parse(challengeWithdraw);
              _stateAccept = int.parse(challengeWithdraw);
            });
