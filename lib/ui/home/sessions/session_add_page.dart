...
class SessionAddPage extends StatefulWidget {
  @override
  _SessionAddPageState createState() => _SessionAddPageState();
}

class _SessionAddPageState extends State<SessionAddPage> implements OptionalDialogListener {
  bool isLoader = false;
  ...
  Widget build(BuildContext context) {
    ...
    return StoreConnector<AppState, _SessionListModel>(
      ...
      appBar:  AppBar(
          ...
          title: Container(
            child: Text(
              DemoLocalizations.of(context).trans('session_add'),
              style: _styleTab,
              textScaleFactor: 1.0,
        ...
        builder: (BuildContext context) {
          return StatefulBuilder(builder: (context, setState) {
            return AlertDialog(
              ...
              title: Text(DemoLocalizations.of(context).trans('dialog_session_act'),
                  style: GoogleFonts.openSans(
          ...
          child:
           Column(
             ...
             children: <Widget>[
               Text.rich(
                 TextSpan(
                   text: DemoLocalizations.of(context).trans('session_date'),
                   style: GoogleFonts.openSans(
                     fontSize: ScreenUtil.getInstance().setWidth(13),
                     color: AppColors.colorGrayDark,
                     fontWeight: FontWeight.w400,
               ...
               TextFormField(
                 ...
                 decoration: new InputDecoration(
                     ...
                     hintText: DemoLocalizations.of(context).trans('select_date'),
                     hintStyle: _hintStyle,
                     prefixText: ' '),
               ),
             ],
...
//  }
    ...
    return
      Container(
        ...
        child: RaisedButton(
          color: AppColors.colorBlue,
          child: Text(
            DemoLocalizations.of(context).trans('forgot_submit'),textScaleFactor: 1.0,
            style: GoogleFonts.openSans(
  ...
  Future<void> _submit() async {
    /*if(_sessionNameController.text.isEmpty){
      ...
      FlutterToast.showToastCenter(DemoLocalizations.of(context).trans('sel_start_date'));
      return;
...
//      FlutterToast.showToastCenter('Form submitted..');
  ...
  Future<void> _addDataBase() async{
    ...
    _sessionData.startTime = _startDateValue + " " + _startTimeController.text;
    int _valueDistanceMetre = int.parse(_distanceController.text)*1000;
    _sessionData.distance = _valueDistanceMetre.toString();
    _sessionData.activityType = transportValue;
    _sessionData.speed = '0';
    _sessionData.sessionType = 'Manual';
    ...
    _session.createdOn = _startDateValue + " " + _startTimeController.text;
    _session.updatedOn = _startDateValue + " " + _endTimeController.text;
    _session.currentDay = _startDateValue ;
    _session.sessionYear = _startDateValue.split("-")[0]+"-"+_startDateValue.split("-")[1];
    ...
    store.state.addSessionList.add(_session);
